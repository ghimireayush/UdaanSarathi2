...
describe('Members Component', () => {
  ...
  describe('Form Validation', () => {
    ...
    it('should have correct role options matching API spec', () => {
      ...
      expect(roleSelect).toContainHTML('<option value="staff">Staff Member</option>');
      expect(roleSelect).toContainHTML('<option value="admin">Administrator</option>');
      expect(roleSelect).toContainHTML('<option value="manager">Manager</option>');
    });
    ...
    it('should validate phone number pattern', () => {
      ...
      expect(phoneInput).toHaveAttribute('placeholder', '10-digit phone number');
    });
  });
  ...
  describe('Member Management', () => {
    ...
    it('should update member status', async () => {
      ...
      await waitFor(() => {
        expect(memberService.updateMemberStatus).toHaveBeenCalledWith('member_1', 'inactive');
      });
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle member loading errors gracefully', async () => {
      memberService.getMembersList.mockRejectedValue(new Error('Failed to load'));
      console.error = jest.fn();

      render(<Members />);

      await waitFor(() => {
        expect(console.error).toHaveBeenCalledWith('Error loading members:', expect.any(Error));
      });
    });
    ...
    it('should handle invitation errors gracefully', async () => {
      const user = userEvent.setup();
      mockConfirm.mockResolvedValue(true);
      memberService.inviteMember.mockRejectedValue(new Error('Invitation failed'));
      console.error = jest.fn();
      ...
      await waitFor(() => {
        expect(console.error).toHaveBeenCalledWith('Error inviting member:', expect.any(Error));
      });
    });
  });
});
