...
const Drafts = () => {
  ...
  useEffect(() => {
    if (headerCheckboxRef.current) {
      headerCheckboxRef.current.indeterminate =
        selectedDrafts.size > 0 && selectedDrafts.size < drafts.length;
    }
  ...
  const handleExpandBulkDraft = async (bulkDraft) => {
    try {
      ...
      const firstEntry =
        bulkDraft.bulk_entries && bulkDraft.bulk_entries.length > 0
      ...
      const convertedDraft = {
        ...
        positions: bulkDraft.bulk_entries
          ? bulkDraft.bulk_entries.map((entry) => ({
      ...
      try {
        ...
        let filteredDrafts = allDrafts.filter((draft) => {
          ...
          const matchesCountry =
            !filters.country || draft.country === filters.country;
          const matchesCompany =
            !filters.company || draft.company === filters.company;
          const matchesCategory =
            !filters.category || draft.category === filters.category;

          return (
            matchesSearch && matchesCountry && matchesCompany && matchesCategory
          );
        });
  ...
  const getDraftStepProgress = (draft) => {
    ...
    const stepCompletionStatus = [
      !!(
        ...
        ((draft.approval_date_ad && draft.posting_date_ad) ||
          (draft.approval_date_bs && draft.posting_date_bs))
      ),

      !!(
        draft.period_years &&
        draft.period_years >= 1 &&
        draft.renewable !== undefined &&
        draft.hours_per_day &&
        draft.hours_per_day >= 1 &&
        draft.hours_per_day <= 16 &&
        draft.days_per_week &&
        draft.days_per_week >= 1 &&
        draft.days_per_week <= 7 &&
        draft.overtime_policy &&
        draft.food &&
        draft.accommodation &&
        draft.transport &&
        draft.annual_leave_days !== undefined &&
        draft.annual_leave_days >= 0
      ),
      ...
      !!(
        ...
        draft.positions.every(
          (pos) =>
            pos.position_title &&
            ((pos.vacancies_male && pos.vacancies_male > 0) ||
              (pos.vacancies_female && pos.vacancies_female > 0)) &&
            pos.monthly_salary &&
            pos.monthly_salary > 0 &&
            pos.currency
        )
      ),
  ...
  const handleDelete = async (draftId) => {
    ...
    setConfirmData({
      id: draftId,
      title: drafts.find((d) => d.id === draftId)?.title,
    });
  ...
  const countries = [
    ...new Set(drafts.map((draft) => draft.country).filter(Boolean)),
  ];
  const companyOptions = [
    ...new Set(drafts.map((draft) => draft.company).filter(Boolean)),
  ];
  const categories = [
    ...new Set(
      drafts.map((draft) => draft.category || draft.title).filter(Boolean)
    ),
  ];
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1 flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span className="flex items-center">
                ...
                {draft.created_at
                  ? format(new Date(draft.created_at), "MMM dd, yyyy")
                  : tPage("table.values.recently")}
              </span>
            </div>
            {!draft.is_bulk_draft && (
              <div className="mt-2">
          ...
          <div className="flex items-center space-x-2 ml-4">
            ...
            >
              {isDeleting
                ? tPage("table.values.deleting")
                : tPage("draftActions.delete")}
            </InteractiveButton>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
          {action === "delete"
            ? tPage("confirmModal.deleteDraft")
            : tPage("confirmModal.editDraft")}
        </h3>
        ...
        <div className="flex justify-end space-x-3">
          <InteractiveButton onClick={onClose} variant="ghost">
  ...
  const sections = [
    {
      ...
      fields: [
        ...
        {
          label: "Approval Date (AD)",
          value: draft.approval_date_ad
            ? new Date(draft.approval_date_ad).toLocaleDateString()
            : null,
          required: true,
        },
        {
          label: "Posting Date (AD)",
          value: draft.posting_date_ad
            ? new Date(draft.posting_date_ad).toLocaleDateString()
            : null,
          required: true,
        },
        {
          label: "Approval Date (BS)",
          value: draft.approval_date_bs,
          required: false,
        },
        {
          label: "Posting Date (BS)",
          value: draft.posting_date_bs,
          required: false,
        },
        {
      ...
      fields: [
        {
          ...
          value: draft.period_years ? `${draft.period_years} years` : null,
          required: true,
        },
        {
        ...
        },
        {
          ...
          value: draft.hours_per_day ? `${draft.hours_per_day} hours` : null,
          required: true,
        },
        {
          label: "Working Days per Week",
          value: draft.days_per_week ? `${draft.days_per_week} days` : null,
          required: true,
        },
        {
          label: "Overtime Policy",
          value: draft.overtime_policy,
          required: true,
        },
        ...
        { label: "Transportation", value: draft.transport, required: true },
        {
          ...
          value: draft.annual_leave_days
            ? `${draft.annual_leave_days} days`
            : null,
          required: true,
        },
        { label: "Visa Status", value: draft.visa_status, required: false },
        {
          label: "Contract Duration Details",
      ...
      fields: [
        {
          ...
          value:
            draft.positions?.length > 0
              ? `${draft.positions.length} position(s) defined`
              : null,
          required: true,
          fullWidth: true,
        },
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-gray-100">
                      {pos.monthly_salary || "N/A"} {pos.currency || "AED"}
                    </p>
                  </div>
                  {pos.job_description && (
      ...
      fields: [
        {
          ...
          value:
            draft.expenses?.length > 0
              ? `${draft.expenses.length} expense item(s)`
              : null,
          required: true,
          fullWidth: true,
        },
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-gray-100">
                      {expense.amount || "N/A"} {expense.currency || "AED"}
                    </p>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700 dark:text-gray-300">
    ...
    },
    {
      ...
      fields: [
        {
          ...
          value:
            ...
            (draft.cutout?.has_file ? "File uploaded" : null),
          required: true,
        },
        {
          label: "File URL",
      ...
      customContent: draft.cutout &&
        (draft.cutout.file_url || draft.cutout.file_name) && (
          <div className="mt-4">
            ...
            {draft.cutout.file_url ? (
              <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                ...
                <div className="hidden text-center text-gray-500 dark:text-gray-400 py-8">
                  ...
                  <p className="text-sm">
                    File: {draft.cutout.file_name || "Unknown"}
                  </p>
                </div>
              </div>
            ) : (
      ...
      fields: [
        ...
        },
        {
          ...
          value: draft.interview_date
            ? new Date(draft.interview_date).toLocaleDateString()
            : null,
          required: false,
        },
        {
    ...
    },
    {
      ...
      fields: [
        {
          label: "Review Status",
          value: draft.review?.is_complete
            ? "Review Completed"
            : "Pending Review",
          required: true,
        },
        ...
        <div className="flex justify-between items-start p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Draft Preview • {draft.company || "No Company"} •{" "}
              {draft.country || "Location TBD"}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="p-6 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                Overall Progress: {progress.completedSteps}/
            ...
            <div className="p-6">
              <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4 mb-6 border border-purple-200 dark:border-purple-800">
                ...
                <p className="text-purple-700 dark:text-purple-300 text-sm">
                  This is a bulk draft containing multiple job positions. Use
                  "Expand Bulk" to convert to individual drafts for detailed
                  editing.
                </p>
              </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Step {section.step + 1} • {status.completedFields}
                              /{status.totalRequired} required fields completed
                              ({status.percentage}%)
                            </p>
                          </div>
                        </div>
            ...
            <div className="flex space-x-3">
              ...
              {!draft.is_bulk_draft && (
                <InteractiveButton
                  onClick={() => {
                    if (!isComplete) {
                      // Show popup/alert when draft is incomplete
                      ...
                      alert(
                        `Please complete all required fields before publishing.\n\n` +
                          ...
                          `Click "Edit Draft" to complete the remaining steps.`
                      );
                      return;
                    }
                    // If complete, proceed with publish
                    onPublish(draft);
                  }}
                  variant="primary"
                  icon={CheckSquare}
