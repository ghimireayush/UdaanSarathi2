 ...
 */
export async function validateAllTranslationFiles(
  ...
  try {
    ...
    for (const locale of locales) {
      ...
      await validateFileIfExists(
        `${translationsDir}/${locale}/common.json`,
        locale,
        "common",
        report
      );
      ...
      for (const pageFile of pageFiles) {
        await validateFileIfExists(
          `${translationsDir}/${locale}/pages/${pageFile}.json`,
          locale,
          `page-${pageFile}`,
          report
        );
      ...
      for (const componentFile of componentFiles) {
        await validateFileIfExists(
          `${translationsDir}/${locale}/components/${componentFile}.json`,
          locale,
...
async function validateFileIfExists(filePath, locale, context, report) {
  try {
    ...
    const validationResult = i18nService.validateTranslations(
      translations,
      context
    );
...
function generateValidationRecommendations(report) {
  ...
  const errorRate =
    report.summary.totalFiles > 0
      ? (report.summary.filesWithErrors / report.summary.totalFiles) * 100
      : 0;
  ...
  if (errorRate > 50) {
    ...
    recommendations.push({
      ...
      message: `${errorRate.toFixed(1)}% of translation files have errors`,
      action:
        "Consider implementing stricter validation in development workflow",
    });
  }
  ...
  if (localeKeys.length > 1) {
    ...
    for (const otherLocale of otherLocales) {
      ...
      const missingFiles = firstLocaleFiles.filter(
        (file) => !otherLocaleFiles.includes(file)
      );
 ...
 */
export async function compareTranslationsBetweenLocales(
  baseLocale = "en",
  compareLocales = ["ne"]
) {
  const report = {
  ...
  try {
    ...
    for (const compareLocale of compareLocales) {
      const compareTranslations = await loadAllTranslationsForLocale(
        compareLocale
      );

      const comparison = compareTranslationStructures(
        baseTranslations,
        compareTranslations,
        baseLocale,
        compareLocale
      ...
      report.summary.structureMismatches +=
        comparison.structureMismatches.length;
    }
  } catch (error) {
  ...
  for (const pageFile of pageFiles) {
    try {
      const pageResponse = await fetch(
        `/translations/${locale}/pages/${pageFile}.json`
      );
      if (pageResponse.ok) {
      ...
      console.warn(
        `Failed to load ${pageFile} translations for ${locale}:`,
        error
      );
    }
  }
 ...
 */
function compareTranslationStructures(
  baseTranslations,
  compareTranslations,
  baseLocale,
  compareLocale
  ...
  if (baseTranslations.pages) {
    for (const [pageName, basePageTranslations] of Object.entries(
      baseTranslations.pages
    )) {
      const comparePageTranslations =
        compareTranslations.pages?.[pageName] || {};
  ...
  for (const [key, value] of Object.entries(baseObj)) {
    ...
    if (!(key in compareObj)) {
      ...
      if (typeof compareObj[key] !== "object" || compareObj[key] === null) {
        ...
        const nestedResult = compareObjectStructures(
          value,
          compareObj[key],
          currentPath
        );
  ...
  for (const [locale, localeData] of Object.entries(report.locales)) {
    for (const [context, fileData] of Object.entries(localeData.files)) {
      ...
      if (fileData.warnings) {
        ciReport.warnings.push(
          ...fileData.warnings.map((warning) => ({
            locale,
            context,
            message: warning,
          }))
