...
const JobDraftWizard = ({ isOpen, onClose, onSave, editingDraft, initialStep = 0 }) => {
    ...
    if (editingDraft && isOpen) {
      ...
      if (editingDraft.is_expanding_bulk && editingDraft.bulk_entries) {
        ...
        setFormData(prevData => ({
          ...
          overtime_policy: editingDraft.overtime_policy || 'as_per_company_policy',
          ...
          positions: [{
            ...
            accommodation_override: '',
            transport_override: '',
            position_notes: `Expanded from bulk draft: ${editingDraft.title || 'Bulk Draft'}`
          }],
        ...
        setFormData(prevData => ({
          ...
          positions: editingDraft.positions && Array.isArray(editingDraft.positions) && editingDraft.positions.length > 0 
            ? editingDraft.positions.map((pos, index) => ({
                ...
                monthly_salary: pos.monthly_salary || pos.salary?.monthly_amount || 0,
  ...
  const singleDraftSteps = [
    { id: 'details', title: 'Posting Details', icon: Users, description: 'Administrative fields' },
    { id: 'contract', title: 'Contract', icon: Briefcase, description: 'Employment terms' },
    { id: 'positions', title: 'Positions', icon: Users, description: 'Job positions with salary' },
    { id: 'tags', title: 'Tags & Canonical Titles', icon: Tag, description: 'Skills, education, experience' },
    { id: 'expenses', title: 'Expenses', icon: DollarSign, description: 'Cost breakdown' },
    { id: 'cutout', title: 'Cutout', icon: Camera, description: 'Job advertisement image' },
    { id: 'interview', title: 'Interview', icon: Users, description: 'Interview process details' },
    { id: 'review', title: 'Review and Publish', icon: CheckCircle, description: 'Final review' }
  ]

  const countries = ['UAE', 'Saudi Arabia', 'Qatar', 'Kuwait', 'Oman', 'Bahrain', 'Malaysia']
  ...
  const citiesByCountry = {
    ...
    'Malaysia': ['Kuala Lumpur', 'Petaling Jaya', 'Shah Alam', 'Subang Jaya', 'Johor Bahru']
  }
  ...
  const predefinedSkills = [
    'Cooking', 'Customer Service', 'Security', 'Surveillance', 'English Communication',
    'Arabic Communication', 'Driving', 'Cleaning', 'Maintenance', 'Electrical Work',
    'Plumbing', 'Construction', 'Welding', 'Carpentry', 'Painting', 'Hospitality',
    'Food Preparation', 'Housekeeping', 'Laundry', 'Gardening', 'Computer Skills',
    'First Aid', 'Leadership', 'Team Work', 'Problem Solving', 'Time Management'
  ]

  const educationRequirements = [
    'No formal education', 'Primary School', 'Class 5', 'Class 8', 'Class 10',
    'SLC/SEE', 'High School', '+2/Intermediate', 'Diploma', 'Vocational Training',
    'Bachelor\'s Degree', 'Master\'s Degree', 'Technical Certification', 'Trade School'
  ]

  const experienceDomains = [
    'Hospitality', 'Culinary', 'Security', 'Construction', 'Maintenance',
    'Transportation', 'Housekeeping', 'Food Service', 'Healthcare', 'Retail',
    'Manufacturing', 'Agriculture', 'Education', 'Office Work', 'Sales',
    'Customer Service', 'Technical Support', 'Guard Duty', 'Surveillance'
  ]
  ...
  const requiredDocuments = [
    'Passport', 'Resume/CV', 'Educational Certificates', 'Experience Certificates',
    'Medical Certificate', 'Police Clearance', 'Driving License', 'Skills Certificate',
    'Photographs', 'Birth Certificate', 'Marriage Certificate', 'Bank Statement',
    'Reference Letters', 'Training Certificates', 'Language Proficiency Certificate',
    'Professional License', 'Insurance Documents', 'Visa Documents'
  ]
  ...
  const validateStep = (step) => {
    ...
    switch(step) {
      case 0: // Posting Details
        ...
        if (!formData.lt_number?.trim()) errors.lt_number = 'Please enter a valid LT Number (e.g., LT-2024-001)'
        if (!formData.chalani_number?.trim()) errors.chalani_number = 'Please enter a valid Chalani Number (e.g., CH-2024-001)'
        if (!formData.country?.trim()) errors.country = 'Country selection is required'
        
        if (formData.date_format === 'AD') {
          if (!formData.approval_date_ad) errors.approval_date_ad = 'Approval Date is required'
          if (!formData.posting_date_ad) errors.posting_date_ad = 'Posting Date is required'
        } else {
          if (!formData.approval_date_bs) errors.approval_date_bs = 'Approval Date is required'
          if (!formData.posting_date_bs) errors.posting_date_bs = 'Posting Date is required'
        }
        
        if (!formData.announcement_type?.trim()) errors.announcement_type = 'Please select an announcement type'
        break
      
      case 1: // Contract
        if (!formData.period_years || formData.period_years < 1) errors.period_years = 'Contract period must be at least 1 year'
        if (!formData.renewable) errors.renewable = 'Please specify if contract is renewable'
        if (!formData.hours_per_day || formData.hours_per_day < 1 || formData.hours_per_day > 16) {
          errors.hours_per_day = 'Working hours must be between 1 and 16 hours per day'
        }
        if (!formData.days_per_week || formData.days_per_week < 1 || formData.days_per_week > 7) {
          errors.days_per_week = 'Working days must be between 1 and 7 days per week'
        }
        if (!formData.overtime_policy?.trim()) errors.overtime_policy = 'Please select an overtime policy'
        if (formData.weekly_off_days < 0 || formData.weekly_off_days > 7) {
          errors.weekly_off_days = 'Weekly off days must be between 0 and 7'
        }
        if (!formData.food?.trim()) errors.food = 'Please specify food provision'
        if (!formData.accommodation?.trim()) errors.accommodation = 'Please specify accommodation provision'
        if (!formData.transport?.trim()) errors.transport = 'Please specify transport provision'
        ...
        if (!formData.positions || formData.positions.length === 0) {
          ...
          formData.positions.forEach((position, index) => {
            if (!position.position_title?.trim()) {
              errors[`positions_${position.id}_position_title`] = 'Position title is required'
            }
            if (position.vacancies_male < 0) {
              errors[`positions_${position.id}_vacancies_male`] = 'Male vacancies must be 0 or greater'
            }
            if (position.vacancies_female < 0) {
              errors[`positions_${position.id}_vacancies_female`] = 'Female vacancies must be 0 or greater'
            }
            if (position.vacancies_male === 0 && position.vacancies_female === 0) {
              errors[`positions_${position.id}_vacancies_total`] = 'Total vacancies must be greater than 0'
            }
            if (!position.monthly_salary || position.monthly_salary <= 0) {
              errors[`positions_${position.id}_monthly_salary`] = 'Monthly salary must be greater than 0'
            }
            if (!position.currency?.trim()) {
              errors[`positions_${position.id}_currency`] = 'Currency selection is required'
            }
          })
        }
        break
      ...
      case 3: // Tags & Titles
        ...
        if (!formData.education_requirements || formData.education_requirements.length === 0) {
          errors.education_requirements = 'Please specify education requirements'
        }
        if (!formData.experience_requirements.min_years || formData.experience_requirements.min_years < 0) {
          errors.experience_min_years = 'Minimum experience years is required (use 0 for no experience)'
        }
        if (!formData.experience_requirements.domains || formData.experience_requirements.domains.length === 0) {
          errors.experience_domains = 'Please specify at least one experience domain'
        }
        if (!formData.canonical_title_ids || formData.canonical_title_ids.length === 0) {
          errors.canonical_titles = 'Please select at least one canonical job title'
        }
        break
        ...
        if (formData.expenses && formData.expenses.length > 0) {
          formData.expenses.forEach((expense) => {
            if (!expense.type?.trim()) {
              errors[`expenses_${expense.id}_type`] = 'Expense type is required'
            }
            if (!expense.who_pays?.trim()) {
              errors[`expenses_${expense.id}_who_pays`] = 'Please specify who pays for this expense'
            }
            if (expense.is_free === false) {
              if (!expense.amount || expense.amount <= 0) {
                errors[`expenses_${expense.id}_amount`] = 'Please enter a valid amount greater than 0'
              }
              if (!expense.currency?.trim()) {
                errors[`expenses_${expense.id}_currency`] = 'Currency is required for paid expenses'
              }
            }
          })
        }
      ...
      case 6: // Interview (Optional)
        if (formData.interview?.date_ad || formData.interview?.date_bs || 
            formData.interview?.time || formData.interview?.location) {
          // If any interview field is filled, validate time format
          if (formData.interview.time && !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]\s?(AM|PM)?$/i.test(formData.interview.time)) {
            errors.interview_time = 'Please enter a valid time format (e.g., 10:00 AM or 14:30)'
          }
        }
        
        // Validate interview expenses if any
        if (formData.interview?.expenses && formData.interview.expenses.length > 0) {
          formData.interview.expenses.forEach((expense) => {
            if (!expense.type?.trim()) {
              errors[`interview_expense_${expense.id}_type`] = 'Expense type is required'
            }
            if (!expense.who_pays?.trim()) {
              errors[`interview_expense_${expense.id}_who_pays`] = 'Please specify who pays'
            }
            if (expense.is_free === false) {
              if (!expense.amount || expense.amount <= 0) {
                errors[`interview_expense_${expense.id}_amount`] = 'Amount is required for paid expenses'
              }
            }
          })
        }
  ...
  const removeExperienceDomain = (domain) => {
    setFormData(prev => ({
      ...
      experience_requirements: {
        ...prev.experience_requirements,
        domains: prev.experience_requirements.domains.filter(item => item !== domain)
      }
    }))
  ...
  const removeCanonicalTitle = (titleId) => {
    ...
    if (titleIndex > -1) {
      setFormData(prev => ({
        ...
        canonical_title_names: prev.canonical_title_names.filter((_, index) => index !== titleIndex)
      }))
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setErrors(prev => ({ ...prev, cutout_file: 'Please select a valid image file (JPG, PNG)' }))
        return
      }
      ...
      if (file.size > maxSize) {
        setErrors(prev => ({ ...prev, cutout_file: 'File size must be less than 10MB' }))
        return
      }
  ...
  const uploadCutout = async () => {
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, cutout_upload: 'Failed to upload cutout. Please try again.' }))
      return false
    }
  }
  ...
  const removeInterviewDocument = (document) => {
    setFormData(prev => ({
      ...
      interview: {
        ...prev.interview,
        required_documents: prev.interview.required_documents.filter(doc => doc !== document)
      }
    }))
  }
  ...
  const removeInterviewExpense = (expenseId) => {
    setFormData(prev => ({
      ...
      interview: {
        ...prev.interview,
        expenses: prev.interview.expenses.filter(exp => exp.id !== expenseId)
      }
    }))
  }
  ...
  const getAIInsights = (step) => {
    ...
    return insights[step] || {
      title: "💡 Keep Going!",
      tips: ["You're doing great! Each step brings you closer to finding the perfect workers for your business."]
    }
  }
  ...
  const validatePostingDetails = () => {
    ...
    if (!formData.lt_number.trim()) newErrors.lt_number = 'LT Number is required'
    if (!formData.chalani_number.trim()) newErrors.chalani_number = 'Chalani Number is required'
    
    if (formData.date_format === 'AD') {
      if (!formData.approval_date_ad) newErrors.approval_date_ad = 'Approval Date is required'
      if (!formData.posting_date_ad) newErrors.posting_date_ad = 'Posting Date is required'
    } else {
      if (!formData.approval_date_bs) newErrors.approval_date_bs = 'Approval Date is required'
      if (!formData.posting_date_bs) newErrors.posting_date_bs = 'Posting Date is required'
    }
  ...
  const validatePositions = () => {
    ...
    formData.positions.forEach((position, index) => {
      ...
      if (!position.position_title.trim()) {
        newErrors[`positions_${positionId}_position_title`] = 'Position Title is required'
      }
      
      if (position.vacancies_male < 0) {
        newErrors[`positions_${positionId}_vacancies_male`] = 'Vacancies (Male) must be 0 or greater'
      }
      
      if (position.vacancies_female < 0) {
        newErrors[`positions_${positionId}_vacancies_female`] = 'Vacancies (Female) must be 0 or greater'
      }
      
      if (position.vacancies_male === 0 && position.vacancies_female === 0) {
        newErrors[`positions_${positionId}_vacancies_total`] = 'Total vacancies must be greater than 0'
      }
      
      if (!position.monthly_salary || position.monthly_salary <= 0) {
        newErrors[`positions_${positionId}_monthly_salary`] = 'Monthly Salary must be greater than 0'
      }
      ...
      if (position.hours_per_day_override && (position.hours_per_day_override < 1 || position.hours_per_day_override > 16)) {
        newErrors[`positions_${positionId}_hours_per_day_override`] = 'Hours Per Day must be between 1 and 16'
      }
      
      if (position.days_per_week_override && (position.days_per_week_override < 1 || position.days_per_week_override > 7)) {
        newErrors[`positions_${positionId}_days_per_week_override`] = 'Days Per Week must be between 1 and 7'
      }
      
      if (position.weekly_off_days_override && (position.weekly_off_days_override < 0 || position.weekly_off_days_override > 7)) {
        newErrors[`positions_${positionId}_weekly_off_days_override`] = 'Weekly Off Days must be between 0 and 7'
      }
    })
  ...
  const validateTagsAndTitles = () => {
    ...
    if (!formData.education_requirements || formData.education_requirements.length === 0) {
      newErrors.education_requirements = 'At least one education requirement is required'
    }
    
    if (!formData.experience_requirements.min_years || formData.experience_requirements.min_years < 0) {
      newErrors.experience_min_years = 'Minimum years of experience is required and must be 0 or greater'
    }
    
    if (formData.experience_requirements.preferred_years && 
        formData.experience_requirements.preferred_years < formData.experience_requirements.min_years) {
      newErrors.experience_preferred_years = 'Preferred years must be greater than or equal to minimum years'
    }
    
    if (!formData.experience_requirements.domains || formData.experience_requirements.domains.length === 0) {
      newErrors.experience_domains = 'At least one experience domain is required'
    }
    
    if (!formData.canonical_title_ids || formData.canonical_title_ids.length === 0) {
      newErrors.canonical_titles = 'At least one canonical job title is required'
    }
  ...
  const validateExpenses = () => {
    ...
    formData.expenses.forEach((expense, index) => {
      ...
      if (expense.is_free === false) {
        if (!expense.amount || expense.amount <= 0) {
          newErrors[`expenses_${expenseId}_amount`] = 'Amount is required when expense is not free and must be greater than 0'
        }
        if (!expense.currency) {
          newErrors[`expenses_${expenseId}_currency`] = 'Currency is required when expense is not free'
        }
      }
    })
  ...
  const validateInterview = () => {
    ...
    if (formData.interview.time && !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]\s?(AM|PM)?$/i.test(formData.interview.time)) {
      newErrors.interview_time = 'Please enter a valid time format (e.g., 10:00 AM or 14:30)'
    }
    ...
    formData.interview.expenses.forEach((expense, index) => {
      ...
      if (!expense.type) {
        newErrors[`interview_expense_${expenseId}_type`] = 'Expense Type is required'
      }
      
      if (!expense.who_pays) {
        newErrors[`interview_expense_${expenseId}_who_pays`] = 'Who Pays is required'
      }
      
      // If expense is not free, amount and currency are required
      if (expense.is_free === false) {
        if (!expense.amount || expense.amount <= 0) {
          newErrors[`interview_expense_${expenseId}_amount`] = 'Amount is required when expense is not free'
        }
        if (!expense.currency) {
          newErrors[`interview_expense_${expenseId}_currency`] = 'Currency is required when expense is not free'
        }
      }
    })
  ...
  const addBulkEntry = () => {
    const newId = Math.max(...formData.bulk_entries.map(entry => entry.id)) + 1
    setFormData(prev => ({
      ...prev,
      bulk_entries: [...prev.bulk_entries, {
        id: newId,
        country: '',
        job_count: '',
        position: ''
  ...
  const handleContinue = () => {
    if (currentFlow === 'bulk') {
      ...
      const bulkData = {
        entries: formData.bulk_entries.filter(entry => entry.country && entry.job_count)
      }
  ...
  const handlePublish = () => {
    ...
    const apiData = {
      ...
      posting_title: formData.posting_title || `${formData.positions[0]?.position_title} Position`,
      ...
      expenses: formData.expenses?.filter(exp => exp.type && exp.who_pays).map(exp => ({
        type: exp.type,
        who_pays: exp.who_pays,
        is_free: exp.is_free,
      ...
      interview: formData.interview?.date_ad || formData.interview?.date_bs ? {
        ...
        expenses: formData.interview.expenses?.filter(exp => exp.type && exp.who_pays) || []
      } : null,
      // Cutout file info
      cutout: formData.cutout?.file || formData.cutout?.uploaded_url ? {
        has_file: Boolean(formData.cutout.file || formData.cutout.uploaded_url),
    ...
    onSave({ type: 'single_publish', data: apiData, original_form_data: formData })
    onClose()
  }
  ...
  const renderBulkCreation = () => (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        {(() => {
          const validEntries = formData.bulk_entries.filter(entry => entry.country && entry.job_count)
          const totalJobs = validEntries.reduce((sum, entry) => sum + (parseInt(entry.job_count) || 0), 0)
    ...
    <div className="space-y-6">
      ...
      <div className="min-h-[400px]">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={formData.date_format === 'AD' ? formData.approval_date_ad : formData.approval_date_bs}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={formData.date_format === 'AD' ? formData.posting_date_ad : formData.posting_date_bs}
                  onChange={(e) => handleInputChange(
        ...
        {currentStep === 1 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-6">Define the employment terms for the job posting.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('period_years', parseInt(e.target.value) || 0)}
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('hours_per_day', parseInt(e.target.value) || 0)}
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('days_per_week', parseInt(e.target.value) || 0)}
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange('overtime_policy', e.target.value)}
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.overtime_policy ? 'border-red-500' : 'border-gray-300'
                  }`}
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('weekly_off_days', parseInt(e.target.value) || 0)}
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.weekly_off_days ? 'border-red-500' : 'border-gray-300'
                  }`}
                  placeholder="1"
                  min="0"
                  max="7"
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.annual_leave_days ? 'border-red-500' : 'border-gray-300'
                  }`}
                  placeholder="21"
                  min="0"
                  max="365"
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Job Positions</h3>
              <p className="text-sm text-gray-600 mb-6">Define one or more job positions, including salary and optional overrides for contract terms.</p>
            </div>

            {errors.positions_general && (
              <div className="bg-red-50 border border-red-200 rounded-md p-3">
                <p className="text-red-600 text-sm">{errors.positions_general}</p>
              </div>
            )}

            {formData.positions.map((position, index) => (
              <div key={position.id} className="border border-gray-200 rounded-lg p-6 space-y-6">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updatePosition(position.id, 'position_title', e.target.value)}
                      className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                        errors[`positions_${position.id}_position_title`] ? 'border-red-500' : 'border-gray-300'
                      }`}
                      placeholder="e.g., Cook"
                    />
                    {errors[`positions_${position.id}_position_title`] && (
                      <p className="text-red-500 text-xs mt-1">{errors[`positions_${position.id}_position_title`]}</p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updatePosition(position.id, 'accommodation_override', e.target.value)}
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        <option value="">Use default ({provisionOptions.find(opt => opt.value === formData.accommodation)?.label})</option>
                        {provisionOptions.map(option => (
                          <option key={option.value} value={option.value}>{option.label}</option>
                        ))}
                      </select>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updatePosition(position.id, 'transport_override', e.target.value)}
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        <option value="">Use default ({provisionOptions.find(opt => opt.value === formData.transport)?.label})</option>
                        {provisionOptions.map(option => (
                          <option key={option.value} value={option.value}>{option.label}</option>
                        ))}
                      </select>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) => updatePosition(position.id, 'position_notes', e.target.value)}
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                        placeholder="e.g., Specific requirements for this position"
                      />
                      <p className="text-xs text-gray-500 mt-1">Optional notes specific to this position</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <select
                    onChange={(e) => {
                      if (e.target.value) {
                        const selectedTitle = jobTitles.find(title => title.id === parseInt(e.target.value))
                        if (selectedTitle) {
            ...
            <div className="space-y-6">
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="font-medium">Contract Period:</span> <span className="ml-2">{formData.period_years} years</span></div>
                  <div className="md:col-span-2">
                    <span className="font-medium">Positions:</span>
                    <span className="ml-2">
                      {formData.positions.filter(p => p.position_title).map(p => `${p.position_title} (${p.monthly_salary} ${p.currency})`).join(', ') || 'No positions defined'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="font-medium">Approval Date:</span> <span className="ml-2">
                    {formData.date_format === 'AD' ? formData.approval_date_ad : formData.approval_date_bs || 'Not specified'}
                    {(formData.approval_date_ad || formData.approval_date_bs) && ` (${formData.date_format})`}
                  </span></div>
                  <div><span className="font-medium">Posting Date:</span> <span className="ml-2">
                    {formData.date_format === 'AD' ? formData.posting_date_ad : formData.posting_date_bs || 'Not specified'}
                    {(formData.posting_date_ad || formData.posting_date_bs) && ` (${formData.date_format})`}
                  </span></div>
                  <div><span className="font-medium">Announcement Type:</span> <span className="ml-2">{formData.announcement_type || 'Not specified'}</span></div>
                  {formData.notes && (
                    <div className="md:col-span-2"><span className="font-medium">Notes:</span> <span className="ml-2">{formData.notes}</span></div>
                  )}
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><span className="font-medium">Days Per Week:</span> <span className="ml-2">{formData.days_per_week}</span></div>
                  <div><span className="font-medium">Overtime Policy:</span> <span className="ml-2">{overtimePolicyOptions.find(opt => opt.value === formData.overtime_policy)?.label}</span></div>
                  <div><span className="font-medium">Weekly Off Days:</span> <span className="ml-2">{formData.weekly_off_days}</span></div>
                  <div><span className="font-medium">Food:</span> <span className="ml-2">{provisionOptions.find(opt => opt.value === formData.food)?.label}</span></div>
                  <div><span className="font-medium">Accommodation:</span> <span className="ml-2">{provisionOptions.find(opt => opt.value === formData.accommodation)?.label}</span></div>
                  <div><span className="font-medium">Transport:</span> <span className="ml-2">{provisionOptions.find(opt => opt.value === formData.transport)?.label}</span></div>
                  <div><span className="font-medium">Annual Leave Days:</span> <span className="ml-2">{formData.annual_leave_days}</span></div>
                </div>
              </div>
                  ...
                  {formData.positions.map((position, index) => (
                    <div key={position.id} className="bg-gray-50 rounded-lg p-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        ...
                        <div><span className="font-medium">Total Vacancies:</span> <span className="ml-2">{position.vacancies_male + position.vacancies_female}</span></div>
                        {(position.hours_per_day_override || position.days_per_week_override || position.overtime_policy_override || 
                          position.weekly_off_days_override || position.food_override || position.accommodation_override || position.transport_override) && (
                          <div className="md:col-span-2">
                            <span className="font-medium">Overrides:</span>
                            <span className="ml-2">
                              {[
                                position.hours_per_day_override && `Hours: ${position.hours_per_day_override}`,
                                position.days_per_week_override && `Days: ${position.days_per_week_override}`,
                                position.overtime_policy_override && `Overtime: ${position.overtime_policy_override}`,
                                position.weekly_off_days_override && `Off Days: ${position.weekly_off_days_override}`,
                                position.food_override && `Food: ${position.food_override}`,
                                position.accommodation_override && `Accommodation: ${position.accommodation_override}`,
                                position.transport_override && `Transport: ${position.transport_override}`
                              ].filter(Boolean).join(', ')}
                            </span>
                          </div>
                        )}
                        {position.position_notes && (
                          <div className="md:col-span-2"><span className="font-medium">Notes:</span> <span className="ml-2">{position.position_notes}</span></div>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2">
                      ...
                      {formData.experience_requirements.preferred_years && `, Preferred: ${formData.experience_requirements.preferred_years} years`}
                    </span>
                    <div className="mt-1 flex flex-wrap gap-1">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                      {formData.canonical_title_names.length > 0 ? formData.canonical_title_names.map(title => (
                        <span key={title} className="px-2 py-1 bg-indigo-100 text-indigo-800 rounded-full text-xs">{title}</span>
                      )) : <span className="text-gray-500">No canonical titles specified</span>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-md font-medium text-gray-800">Expenses ({formData.expenses.length})</h4>
                  <button
                ...
                <div className="space-y-3">
                  {formData.expenses.map((expense, index) => (
                    <div key={expense.id} className="bg-gray-50 rounded-lg p-3 text-sm">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div><span className="font-medium">Type:</span> <span className="ml-2">{expenseTypes.find(t => t.value === expense.type)?.label || expense.type}</span></div>
                        <div><span className="font-medium">Who Pays:</span> <span className="ml-2">{expensePayers.find(p => p.value === expense.who_pays)?.label || expense.who_pays}</span></div>
                        <div><span className="font-medium">Cost:</span> <span className="ml-2">
                          {expense.is_free ? 'Free' : `${expense.amount} ${expense.currency}`}
                        </span></div>
                        {expense.notes && (
                          <div className="md:col-span-3"><span className="font-medium">Notes:</span> <span className="ml-2">{expense.notes}</span></div>
                        )}
                      </div>
                    </div>
                  ))}
                  {formData.expenses.length === 0 && (
                    <p className="text-gray-500 text-sm">No expenses specified</p>
                  )}
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-md font-medium text-gray-800">Job Advertisement Image</h4>
                  <button
                ...
                {formData.cutout.preview_url || formData.cutout.uploaded_url ? (
                  <div className="flex items-center space-x-4">
                    <img
                      src={formData.cutout.preview_url || formData.cutout.uploaded_url}
                      alt="Job Advertisement"
                      className="w-24 h-24 object-cover rounded-lg border border-gray-300"
                    />
                    <div className="text-sm">
                      <div><span className="font-medium">Status:</span> <span className="ml-2">
                        {formData.cutout.is_uploaded ? 'Uploaded' : 'Ready to Upload'}
                      </span></div>
                      {formData.cutout.file && (
                        <div><span className="font-medium">File:</span> <span className="ml-2">{formData.cutout.file.name}</span></div>
                      )}
                    </div>
                  </div>
                ) : (
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-md font-medium text-gray-800">Interview Process</h4>
                  <button
                ...
                {(formData.interview.date_ad || formData.interview.date_bs || formData.interview.time || 
                  formData.interview.location || formData.interview.contact_person || 
                  formData.interview.required_documents.length > 0 || formData.interview.notes ||
                  formData.interview.expenses.length > 0) ? (
                  <div className="space-y-3 text-sm">
                    {(formData.interview.date_ad || formData.interview.date_bs) && (
                      <div><span className="font-medium">Date:</span> <span className="ml-2">
                        {formData.interview.date_format === 'AD' ? formData.interview.date_ad : formData.interview.date_bs}
                        {` (${formData.interview.date_format})`}
                      </span></div>
                    )}
                    {formData.interview.time && (
                      <div><span className="font-medium">Time:</span> <span className="ml-2">{formData.interview.time}</span></div>
                    )}
                    {formData.interview.location && (
                      <div><span className="font-medium">Location:</span> <span className="ml-2">{formData.interview.location}</span></div>
                    )}
                    {formData.interview.contact_person && (
                      <div><span className="font-medium">Contact Person:</span> <span className="ml-2">{formData.interview.contact_person}</span></div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {formData.interview.required_documents.map(doc => (
                            <span key={doc} className="px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs">{doc}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {formData.interview.notes && (
                      <div><span className="font-medium">Notes:</span> <span className="ml-2">{formData.interview.notes}</span></div>
                    )}
                    {formData.interview.expenses.length > 0 && (
                      <div>
                        <span className="font-medium">Interview Expenses ({formData.interview.expenses.length}):</span>
                        <div className="mt-2 space-y-2">
                          {formData.interview.expenses.map((expense, index) => (
                            <div key={expense.id} className="bg-gray-50 rounded p-2">
                              <div className="text-xs">
                                <span className="font-medium">{expenseTypes.find(t => t.value === expense.type)?.label}</span>
                                <span className="ml-2">({expensePayers.find(p => p.value === expense.who_pays)?.label})</span>
                                <span className="ml-2">{expense.is_free ? 'Free' : `${expense.amount} ${expense.currency}`}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <p className="text-gray-500 text-sm">No interview details specified</p>
                )}
              </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Job Advertisement Image</h3>
              <p className="text-sm text-gray-600 mb-6">Upload and manage the job advertisement image (e.g., newspaper scan).</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Upload File <span className="text-sm text-gray-500 font-normal">(Optional)</span>
                  <HelpCircle className="inline w-4 h-4 ml-1 text-gray-400 cursor-help" title="Upload a job advertisement image like a newspaper scan or digital poster" />
                </label>
                
                {/* File Input */}
                <div className="mt-2">
                  <div className="flex items-center justify-center w-full">
                    <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 hover:border-gray-400 transition-colors">
                ...
                {errors.cutout_file && (
                  <p className="text-red-500 text-xs mt-1">{errors.cutout_file}</p>
                )}
                {errors.cutout_upload && (
                  <p className="text-red-500 text-xs mt-1">{errors.cutout_upload}</p>
                )}
                
                <p className="text-xs text-gray-500 mt-1">Supported formats: JPG, PNG. Maximum file size: 10MB</p>
              </div>
              ...
              {(formData.cutout.preview_url || formData.cutout.uploaded_url) && (
                <div className="border border-gray-200 rounded-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-md font-medium text-gray-800">Image Preview</h4>
                    <div className="flex space-x-2">
                      ...
                      <div className="flex space-x-1">
                        <button
                          onClick={() => {
                            if (window.confirm('Are you sure you want to remove this image? (Soft delete - file remains on disk)'))
                              removeCutout(false)
                          }}
                      ...
                      {formData.cutout.file && (
                        <>
                          <div>
                            ...
                            <span className="ml-2 text-gray-600">{formData.cutout.file.name}</span>
                          </div>
                          <div>
                            <span className="font-medium text-gray-700">File Size:</span>
                            <span className="ml-2 text-gray-600">{(formData.cutout.file.size / 1024 / 1024).toFixed(2)} MB</span>
                          </div>
                          <div>
                            <span className="font-medium text-gray-700">File Type:</span>
                            <span className="ml-2 text-gray-600">{formData.cutout.file.type}</span>
                          </div>
                        </>
                      )}
                      <div>
        ...
        {currentStep === 6 && (
          <div className="space-y-6">
            ...
            <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => updateInterviewField(
              ...
              <div className="border border-gray-200 rounded-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-md font-medium text-gray-800">Interview-Related Expenses</h4>
                  <button
                ...
                {formData.interview.expenses.length === 0 ? (
                  <p className="text-gray-500 text-sm text-center py-4">No interview expenses added yet</p>
                ) : (
                  <div className="space-y-4">
                    {formData.interview.expenses.map((expense, index) => (
                      <div key={expense.id} className="border border-gray-100 rounded-lg p-4 bg-gray-50">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-sm font-medium text-gray-700">Interview Expense {index + 1}</span>
                          <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <select
                              value={expense.type}
                              onChange={(e) => updateInterviewExpense(expense.id, 'type', e.target.value)}
                          ...
                          <div>
                            ...
                            <select
                              value={expense.who_pays}
                              onChange={(e) => updateInterviewExpense(expense.id, 'who_pays', e.target.value)}
                              className="w-full text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                            >
                            ...
                            <div className="flex space-x-2">
                              <label className="flex items-center text-sm">
                                <input
                                  ...
                                  checked={expense.is_free === true}
                                  onChange={() => updateInterviewExpense(expense.id, 'is_free', true)}
                              ...
                              <label className="flex items-center text-sm">
                                <input
                                  ...
                                  name={`interview_expense_free_${expense.id}`}
                                  checked={expense.is_free === false}
                                  onChange={() => updateInterviewExpense(expense.id, 'is_free', false)}
                                  className="mr-1 text-blue-600"
                                />
                                No
                              </label>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                            <div>
                              ...
                              <input
                                type="number"
                                value={expense.amount}
                                onChange={(e) => updateInterviewExpense(expense.id, 'amount', parseFloat(e.target.value) || '')}
                                className="w-full text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                                placeholder="0"
                                min="0"
                                step="0.01"
                            ...
                            <div>
                              ...
                              <select
                                value={expense.currency}
                                onChange={(e) => updateInterviewExpense(expense.id, 'currency', e.target.value)}
                                className="w-full text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                              >
                                {currencies.map(curr => (
                                  <option key={curr.value} value={curr.value}>{curr.label}</option>
                        ...
                        <div className="mt-3">
                          ...
                          <textarea
                            rows={2}
                            value={expense.notes}
                            onChange={(e) => updateInterviewExpense(expense.id, 'notes', e.target.value)}
